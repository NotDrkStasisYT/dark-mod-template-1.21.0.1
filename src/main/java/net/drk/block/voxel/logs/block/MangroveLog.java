...
import net.drk.block.voxel.logs.blockEntity.MangroveLogBlockEntity;
...
public class MangroveLog extends Block implements BlockEntityProvider {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof MangroveLogBlockEntity mangroveLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    mangroveLogBlockEntity.damageBlock(damage);
                }

                if (mangroveLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new MangroveLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.MANGROVE_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof MangroveLogBlockEntity mangroveLogBlockEntity) {
                    MangroveLogBlockEntity.tick(world1, pos, state1, mangroveLogBlockEntity);
                }
            };
        }
        return null;
