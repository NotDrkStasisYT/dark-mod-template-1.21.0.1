...
import net.drk.block.voxel.logs.blockEntity.ShadowLogBlockEntity;
...
public class ShadowLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public ShadowLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof ShadowLogBlockEntity shadowLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    shadowLogBlockEntity.damageBlock(damage);
                }

                if (shadowLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new ShadowLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.SHADOW_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof ShadowLogBlockEntity shadowLogBlockEntity) {
                    ShadowLogBlockEntity.tick(world1, pos, state1, shadowLogBlockEntity);
                }
            };
        }
        return null;
