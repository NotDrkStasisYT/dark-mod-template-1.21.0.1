...
import net.drk.block.voxel.logs.blockEntity.OakLogBlockEntity;
...
public class OakLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public OakLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof OakLogBlockEntity oakLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    oakLogBlockEntity.damageBlock(damage);
                }

                if (oakLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.OAK_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof OakLogBlockEntity oakLogBlockEntity) {
                    OakLogBlockEntity.tick(world1, pos, state1, oakLogBlockEntity);
                }
            };
        }
        return null;
