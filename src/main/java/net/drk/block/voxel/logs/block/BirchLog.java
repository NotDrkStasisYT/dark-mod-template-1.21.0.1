...
import net.drk.block.voxel.logs.blockEntity.BirchLogBlockEntity;
...
public class BirchLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public BirchLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof BirchLogBlockEntity birchLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    birchLogBlockEntity.damageBlock(damage);
                }

                if (birchLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new BirchLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.BIRCH_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof BirchLogBlockEntity birchLogBlockEntity) {
                    BirchLogBlockEntity.tick(world1, pos, state1, birchLogBlockEntity);
                }
            };
        }
        return null;
