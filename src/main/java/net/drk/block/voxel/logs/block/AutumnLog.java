...
import net.drk.block.voxel.logs.blockEntity.AutumnLogBlockEntity;
...
public class AutumnLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public AutumnLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof AutumnLogBlockEntity autumnLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    autumnLogBlockEntity.damageBlock(damage);
                }

                if (autumnLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new AutumnLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.AUTUMN_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof AutumnLogBlockEntity autumnLogBlockEntity) {
                    AutumnLogBlockEntity.tick(world1, pos, state1, autumnLogBlockEntity);
                }
            };
        }
        return null;
