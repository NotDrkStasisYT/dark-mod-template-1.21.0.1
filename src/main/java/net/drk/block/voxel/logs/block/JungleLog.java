...
import net.drk.block.voxel.logs.blockEntity.JungleLogBlockEntity;
...
public class JungleLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public JungleLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof JungleLogBlockEntity jungleLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    jungleLogBlockEntity.damageBlock(damage);
                }

                if (jungleLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new JungleLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.JUNGLE_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof JungleLogBlockEntity jungleLogBlockEntity) {
                    JungleLogBlockEntity.tick(world1, pos, state1, jungleLogBlockEntity);
                }
            };
        }
        return null;
