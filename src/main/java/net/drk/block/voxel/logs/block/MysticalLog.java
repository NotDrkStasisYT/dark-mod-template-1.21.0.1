...
import net.drk.block.voxel.logs.blockEntity.MysticalLogBlockEntity;
...
public class MysticalLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public MysticalLog(Settings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(ROTATION, 0)); // Default rotation 0
    }

    public static int getLighting(BlockState currentBlockState) {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof MysticalLogBlockEntity mysticalLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    mysticalLogBlockEntity.damageBlock(damage);
                }

                if (mysticalLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new MysticalLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.MYSTICAL_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof MysticalLogBlockEntity mysticalLogBlockEntity) {
                    MysticalLogBlockEntity.tick(world1, pos, state1, mysticalLogBlockEntity);
                }
            };
        }
        return null;
