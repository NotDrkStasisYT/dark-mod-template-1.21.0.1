...
public class SpruceLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public SpruceLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof SpruceLogBlockEntity spruceLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    spruceLogBlockEntity.damageBlock(damage);
                }

                if (spruceLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SpruceLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.SPRUCE_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof SpruceLogBlockEntity spruceLogBlockEntity) {
                    SpruceLogBlockEntity.tick(world1, pos, state1, spruceLogBlockEntity);
                }
            };
        }
        return null;
