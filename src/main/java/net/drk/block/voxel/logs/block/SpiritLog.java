...
import net.drk.block.voxel.logs.blockEntity.SpiritLogBlockEntity;
...
public class SpiritLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public SpiritLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof SpiritLogBlockEntity spiritLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    spiritLogBlockEntity.damageBlock(damage);
                }

                if (spiritLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SpiritLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.SPIRIT_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof SpiritLogBlockEntity spiritLogBlockEntity) {
                    SpiritLogBlockEntity.tick(world1, pos, state1, spiritLogBlockEntity);
                }
            };
        }
        return null;
