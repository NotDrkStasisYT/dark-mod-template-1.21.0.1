...
import net.drk.block.voxel.logs.blockEntity.CherryLogBlockEntity;
...
public class CherryLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public CherryLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof CherryLogBlockEntity cherryLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    cherryLogBlockEntity.damageBlock(damage);
                }

                if (cherryLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new CherryLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.CHERRY_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof CherryLogBlockEntity cherryLogBlockEntity) {
                    CherryLogBlockEntity.tick(world1, pos, state1, cherryLogBlockEntity);
                }
            };
        }
        return null;
