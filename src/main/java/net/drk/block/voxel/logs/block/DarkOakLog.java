...
import net.drk.block.voxel.logs.blockEntity.DarkOakLogBlockEntity;
...
public class DarkOakLog extends Block implements BlockEntityProvider {
    ...
    // Constructor
    public DarkOakLog(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            ...
            if (entity instanceof DarkOakLogBlockEntity darkOakLogBlockEntity) {
                ...
                if (isOnCooldown(player, weaponKey)) {
                    ...
                    darkOakLogBlockEntity.damageBlock(damage);
                }

                if (darkOakLogBlockEntity.isDestroyed()) {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new DarkOakLogBlockEntity(pos, state);
    }

    @Override
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (type == ModBlockEntities.DARK_OAK_LOG) {
            return (world1, pos, state1, entity) -> {
                if (entity instanceof DarkOakLogBlockEntity darkOakLogBlockEntity) {
                    DarkOakLogBlockEntity.tick(world1, pos, state1, darkOakLogBlockEntity);
                }
            };
        }
        return null;
